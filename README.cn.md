# csvi

[English version](./README.md)

Csvi - A terminal CSV editor

[![x-cmd/install — csvi Code Quality Monitoring Repo Card](https://repo.x-cmd.io/csvi.svg?lang=zh)](https://x-cmd.com/install/csvi)

## 安装

```sh
x install csvi
```

## 代码洞察

合计: **6,509** 行代码（覆盖前 5 种语言、共 **44** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Go | 4,335 | 52 | 433 | 37 |
| Html | 2,080 | 6 | 17 | 4 |
| Makefile | 55 | 0 | 14 | 1 |
| Json | 28 | 0 | 0 | 1 |
| PowerShell | 11 | 0 | 1 | 1 |

## 源代码

- **上游仓库**: <https://github.com/hymkor/csvi>
- **官网**: <https://hymkor.github.io/csvi/>
- **许可证**: MIT

## 发布

- **最新版本**: `v1.23.3` (2026-08-26)
- **最近提交**: 2026-08-26
- **Release 含资产**: 7 个

## 流行度

- **Star**: 188 · **Fork**: 5 · **开放 issue**: 12 · **贡献者**: 1

## 累计统计

- **发布数**: 49 · **已合并 PR**: 103 · **开放 PR**: 0 · **已关闭 issue**: 11 · **开放 issue**: 1 · **提交数**: 976

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-21 | 1 | 2 | 0 | 0 | 1 | 0 |
| last60d | 2026-07-22 | 1 | 2 | 0 | 0 | 1 | 8 |
| 90d | 2026-06-22 | 1 | 2 | 0 | 0 | 1 | 8 |
| last180d | 2026-03-24 | 2 | 6 | 0 | 0 | 1 | 21 |
| 360d | 2025-09-25 | 21 | 102 | 0 | 8 | 1 | 392 |
| last720d | 2024-09-30 | 26 | 102 | 0 | 9 | 1 | 563 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [csvi-v1.23.3-darwin-amd64.zip](https://github.com/hymkor/csvi/releases/download/v1.23.3/csvi-v1.23.3-darwin-amd64.zip) | 1.4 MiB | `native/darwin/x64` |
| [csvi-v1.23.3-darwin-arm64.zip](https://github.com/hymkor/csvi/releases/download/v1.23.3/csvi-v1.23.3-darwin-arm64.zip) | 1.4 MiB | `native/darwin/arm64` |
| [csvi-v1.23.3-freebsd-amd64.zip](https://github.com/hymkor/csvi/releases/download/v1.23.3/csvi-v1.23.3-freebsd-amd64.zip) | 1.4 MiB | `other` |
| [csvi-v1.23.3-linux-386.zip](https://github.com/hymkor/csvi/releases/download/v1.23.3/csvi-v1.23.3-linux-386.zip) | 1.4 MiB | `other` |
| [csvi-v1.23.3-linux-amd64.zip](https://github.com/hymkor/csvi/releases/download/v1.23.3/csvi-v1.23.3-linux-amd64.zip) | 1.4 MiB | `native/linux/x64` |
| [csvi-v1.23.3-windows-386.zip](https://github.com/hymkor/csvi/releases/download/v1.23.3/csvi-v1.23.3-windows-386.zip) | 1.5 MiB | `native/win/x64` |
| [csvi-v1.23.3-windows-amd64.zip](https://github.com/hymkor/csvi/releases/download/v1.23.3/csvi-v1.23.3-windows-amd64.zip) | 1.5 MiB | `native/win/x64` |

## 改进这些数据

csvi 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `csvi` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/csvi.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260920.yml` · 2026-09-20T05:30:50Z._
